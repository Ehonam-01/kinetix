...
import type { RevenuePoint } from "@/repositories/admin-stats";
...
// Same shape as components/balance-evolution-chart.tsx (mirrored rather
// than shared, deliberately — that one backs the member dashboard's real
// balance display, kept untouched here so this admin-only addition can't
// regress it). Chiffre d'affaires cumulé sur 30 jours — une seule série,
// pas de légende nécessaire (le titre de la carte la porte déjà). Le point
// final est étiqueté directement, la valeur totale reste dans les
// StatCards au-dessus même sans survoler le graphique.
export function AdminRevenueChart({ points }: { points: RevenuePoint[] }) {
  const gradientId = useId();
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);

  const hasActivity = points.some((p) => p.cumulativeRevenue !== 0);

  const { linePath, areaPath, xy, min, max } = useMemo(() => {
    const values = points.map((p) => p.cumulativeRevenue);
    ...
    const xy = points.map((p, i) => ({
      ...
      y:
        PAD_TOP +
        CHART_HEIGHT -
        ((p.cumulativeRevenue - min) / (max - min)) * CHART_HEIGHT,
    }));
  ...
  if (points.length === 0 || !hasActivity) {
    return (
      <div className="text-muted-foreground flex h-[200px] items-center justify-center text-sm">
        Aucun chiffre d&apos;affaires sur les 30 derniers jours.
      </div>
    );
  }
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={`Chiffre d'affaires cumulé sur les 30 derniers jours, de ${formatXof(points[0].cumulativeRevenue)} à ${formatXof(last.cumulativeRevenue)}`}
    >
      <defs>
      ...
      >
        {formatXof(last.cumulativeRevenue)}
      </text>
      ...
      {hoveredXy && hovered && (
        <>
          ...
          <g transform={`translate(${tooltipX}, ${PAD_TOP})`}>
            ...
            >
              {formatXof(hovered.cumulativeRevenue)}
            </text>
            <text x={8} y={28} fontSize={9} fill="var(--color-muted-foreground)">
